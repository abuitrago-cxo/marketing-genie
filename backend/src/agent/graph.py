...
if not all(k in os.environ for k in ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_REGION", "BRAVE_SEARCH_API_KEY"]):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning(f"Search attempt {attempt + 1} failed: {str(e)}. Retrying in {delay:.2f}s...")
                time.sleep(delay)
            else:
                raise e

def parse_structured_output(response_text: str, schema_type: str):
    ...
    try:
        ...
        logger.warning(f"Failed to parse {schema_type} response: {str(e)}")
        return None
...
def web_research(state: WebSearchState, config: RunnableConfig) -> OverallState:
    ...
    try:
        ...
        return {
            ...
            "web_research_result": [f"Search failed for query '{state['search_query']}': {str(e)}"],
        }
...
"""
        ...
        if parsed and "is_sufficient" in parsed:
            logger.info(f"Reflection result - sufficient: {parsed['is_sufficient']}, follow-up queries: {parsed.get('follow_up_queries', [])}")
            return {
        ...
        research_topic = get_research_topic(state["messages"])
        return {
            "is_sufficient": state["research_loop_count"] >= 2,  # Default to sufficient after 2 loops
            "knowledge_gap": "Error in reflection process.",
            "follow_up_queries": [research_topic] if state["research_loop_count"] < 2 else [],
            "research_loop_count": state["research_loop_count"],
            "number_of_ran_queries": len(state["search_query"]),
        }
...
def evaluate_research(
    ...
    logger.info(f"Evaluating research: sufficient={state['is_sufficient']}, loop={state['research_loop_count']}, max={max_research_loops}")
